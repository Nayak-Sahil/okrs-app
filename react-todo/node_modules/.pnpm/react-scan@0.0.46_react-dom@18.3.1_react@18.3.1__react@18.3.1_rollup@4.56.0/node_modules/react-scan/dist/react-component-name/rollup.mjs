...
var createBabelPlugin = () => {
  ...
  function isReactComponent(path) {
    ...
    if (t.isClassDeclaration(path.node) && path.node.superClass && t.isMemberExpression(path.node.superClass) && t.isIdentifier(path.node.superClass.object) && path.node.superClass.object.name === "React" && t.isIdentifier(path.node.superClass.property) && path.node.superClass.property.name === "Component") {
      return true;
    }
    if (t.isCallExpression(path.node)) {
      ...
      if (t.isMemberExpression(callee) && t.isIdentifier(callee.object) && callee.object.name === "React" && t.isIdentifier(callee.property) && callee.property.name === "createClass") {
        return true;
      }
      ...
      if (t.isMemberExpression(callee)) {
        return t.isIdentifier(callee.object) && callee.object.name === "React" && t.isIdentifier(callee.property) && ["memo", "forwardRef", "createClass"].includes(callee.property.name);
      }
    }
    return false;
  }
        ...
        exit(path) {
          ...
          path.traverse({
            AssignmentExpression(path2) {
              const { node } = path2;
              if (t.isMemberExpression(node.left) && t.isIdentifier(node.left.property) && node.left.property.name === "displayName" && t.isIdentifier(node.left.object)) {
          ...
          path.traverse({
            "ClassDeclaration|FunctionDeclaration|VariableDeclarator"(path2) {
              ...
              if (componentName && isComponentName(componentName) && !hasDisplayNameAssignment.has(componentName) && isReactComponent(componentPath)) {
